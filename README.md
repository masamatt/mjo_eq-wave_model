...
## V. Model Description
The two models ([primitive equation](#vi-primitive-equation-model), [balanced](#vii-balanced-model)) described below comprise the original model code.  The plots for <sup>[\[1\]](#1), </sup><sup>[\[2\]](#2)</sup> were produced at the time by Brian McNoldy using IDL software.  The core model code remains unchanged, though  plotting scripts have now been added to automatically plot the output within Matlab.  These scripts have been made to emulate as close as possible the look of the original IDL plots, though are formatted in three panels which display all five of the dependent variable fields solved for.
