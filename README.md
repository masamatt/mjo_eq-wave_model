...
equations in `(x,y,z,t)`-space are separated into horizonal `(x,y,t)` and vertical (`z`) structure equations in [Sec 2.2](https://github.com/masamatt/mjo_eq-wave_model/blob/master/docs/mtm_thesis_sec_2.2.pdf) by way of a [vertical normal mode transform](https://github.com/masamatt/mjo_eq-wave_model/blob/master/docs/mtm_thesis_vert_norm_mode.pdf).  Justified the comparison of [observed vertical heating profiles](LINK ME!!!!!) with the first vertical internal mode of the transform, we make the simplifying assumption that the heating excites only the first internal mode.  In this way the infinite series of vertical modes is truncated to a single mode.  In 
