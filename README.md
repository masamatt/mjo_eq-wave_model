...
Only 3 scripts, all located in the root directory, are needed by the user to operate the model.  The first [MJO_RUN.m](#iiiii---run-model--mjo_runm) querys for simulation type, then calls the run.  The second [MJO_PARAMS.m](#iiiiii---set-run-parameters--mjo_paramsm) contains the user adjustable parameters controlling physical aspects of a simulation.  The last [MJO_PLOT.m](#iiiiv---plot-model-output--mjo_plotm) contains user adjustable parameters for plot appareance and makes the plotting call as well.<br>
...
*NOTE:  the parameter `zonalDomain` specifies the zonal physical extent to **display**.  The solutions computed in spectral space are  always solved for the **full equatorial channel**.  Plotting just half of the circumference (`zonalDomain = 0.5`), centered on the diabatic forcing, allows for more resolution in the region of most interest.*
<br><br>
