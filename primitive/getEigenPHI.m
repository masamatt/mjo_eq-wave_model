...
% DATE:        June 29, 2005
% MODIFIED:    N/A
% CALL SYNTAX: PHI = getEigenPHI(A,Nu,H0,H,yHatVec,M,N);
...
PHI = zeros(ySize,mSize,nSize,rSize);
      ...
      for kr = 1:rSize    % r loop
        ...
        if nVec(jn) > 1
          ...
          % ------------------
            ...
            PHI(yy,im,jn,kr)=A(im,jn,kr)*cBar*ep^(1/4)*(T1a*T1b-T2a*T2b);
          end
        ...
        elseif nVec(jn) == 0
          ...
          if rVec(kr) ~= 1
            ...
            T2 = 2^(-1/2)*H(yy,1);
            PHI(yy,im,jn,kr) = A(im,jn,kr)*cBar*ep^(1/4)*T1*T2;
          % r = 1
          % -----
          else
            PHI(yy,im,jn,kr) = -9999;
          end
